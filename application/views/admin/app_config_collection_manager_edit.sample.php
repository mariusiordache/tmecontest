...
<?php include($crud_ignition_views_path . 'header.php'); ?><!-- END header -->
		...
		<h1 style="margin-bottom: 20px;">app_config</h1>
		...
		<!-- field: config -->
		<label style="font-weight: bold; color: #555; margin-top: 10px;">config</label>
		<?php echo form_error('config', '<div class="alert alert-error">', '</div>'); ?>
		<?php echo form_textarea('config', set_value('config', $item['config']), ' id="field_config" '); ?>
		<label class="checkbox inline" style="margin-left: 10px; margin-top: -15px;"><?php echo form_checkbox('','', isset($_POST['ignore_field_config']) ? true : false, ' class="toggler" data-field="field_config" '); ?> Ignore</label>
