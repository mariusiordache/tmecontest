...
<?php include($crud_ignition_views_path . 'header.php'); ?><!-- END header -->
		...
		<div class="row-fluid">
			...
			<form method="get" class="form form-inline pull-right">
				...
				<select name="search_field" class="input-medium">
					...
					<option value="id" <?php if($search_field=='id') { echo 'selected="selected"'; } ?> >id</option><option value="story_id" <?php if($search_field=='story_id') { echo 'selected="selected"'; } ?> >story_id</option><option value="position" <?php if($search_field=='position') { echo 'selected="selected"'; } ?> >position</option><option value="paragraph" <?php if($search_field=='paragraph') { echo 'selected="selected"'; } ?> >paragraph</option><option value="date_created" <?php if($search_field=='date_created') { echo 'selected="selected"'; } ?> >date_created</option>				</select>
				<label>Sort</label>				
				<select name="sort_field" class="input-small">
					<option value="">Sort by ...</option>
					<option value="id" <?php if($sort_field=='id') { echo 'selected="selected"'; } ?> >id</option><option value="story_id" <?php if($sort_field=='story_id') { echo 'selected="selected"'; } ?> >story_id</option><option value="position" <?php if($sort_field=='position') { echo 'selected="selected"'; } ?> >position</option><option value="paragraph" <?php if($sort_field=='paragraph') { echo 'selected="selected"'; } ?> >paragraph</option><option value="date_created" <?php if($sort_field=='date_created') { echo 'selected="selected"'; } ?> >date_created</option>				</select>
		...
		<table class="table table-bordered table-striped">
			<thead>
				...
				<th>id</th><th>story_id</th><th>position</th><th>paragraph</th><th>date_created</th>					<td></td>
				</tr>
			</thead>
			<tbody>
				...
				<?php if($fields['id']['admin_header'] == 'on') { echo '<td>'.$item['id'].'</td>';  } ?><?php if($fields['story_id']['admin_header'] == 'on') { echo '<td>'.$item['story_id'].'</td>';  } ?><?php if($fields['position']['admin_header'] == 'on') { echo '<td>'.$item['position'].'</td>';  } ?><?php if($fields['paragraph']['admin_header'] == 'on') { echo '<td>'.$item['paragraph'].'</td>';  } ?><?php if($fields['date_created']['admin_header'] == 'on') { echo '<td>'.$item['date_created'].'</td>';  } ?>					<td>	
