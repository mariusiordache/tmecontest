...
<?php include($crud_ignition_views_path . 'header.php'); ?><!-- END header -->
		...
		<div class="row-fluid">
			...
			<form method="get" class="form form-inline pull-right">
				...
				<select name="search_field" class="input-medium">
					...
					<option value="id" <?php if($search_field=='id') { echo 'selected="selected"'; } ?> >id</option><option value="name" <?php if($search_field=='name') { echo 'selected="selected"'; } ?> >name</option><option value="config" <?php if($search_field=='config') { echo 'selected="selected"'; } ?> >config</option>				</select>
				<label>Sort</label>				
				<select name="sort_field" class="input-small">
					<option value="">Sort by ...</option>
					<option value="id" <?php if($sort_field=='id') { echo 'selected="selected"'; } ?> >id</option><option value="name" <?php if($sort_field=='name') { echo 'selected="selected"'; } ?> >name</option><option value="config" <?php if($sort_field=='config') { echo 'selected="selected"'; } ?> >config</option>				</select>
		...
		<table class="table table-bordered table-striped">
			<thead>
				...
				<th>id</th><th>name</th><th>config</th>					<td></td>
				</tr>
			</thead>
			<tbody>
				...
				<?php if($fields['id']['admin_header'] == 'on') { echo '<td>'.$item['id'].'</td>';  } ?><?php if($fields['name']['admin_header'] == 'on') { echo '<td>'.$item['name'].'</td>';  } ?><?php if($fields['config']['admin_header'] == 'on') { echo '<td>'.$item['config'].'</td>';  } ?>					<td>	
