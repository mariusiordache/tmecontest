...
<?php include($crud_ignition_views_path . 'header.php'); ?><!-- END header -->
		...
		<div class="row-fluid">
			...
			<form method="get" class="form form-inline pull-right">
				...
				<select name="search_field" class="input-medium">
					...
					<option value="id" <?php if($search_field=='id') { echo 'selected="selected"'; } ?> >id</option><option value="hash" <?php if($search_field=='hash') { echo 'selected="selected"'; } ?> >hash</option><option value="name" <?php if($search_field=='name') { echo 'selected="selected"'; } ?> >name</option>				</select>
				<label>Sort</label>				
				<select name="sort_field" class="input-small">
					<option value="">Sort by ...</option>
					<option value="id" <?php if($sort_field=='id') { echo 'selected="selected"'; } ?> >id</option><option value="hash" <?php if($sort_field=='hash') { echo 'selected="selected"'; } ?> >hash</option><option value="name" <?php if($sort_field=='name') { echo 'selected="selected"'; } ?> >name</option>				</select>
		...
		<table class="table table-bordered table-striped">
			<thead>
				...
				<th>id</th><th>hash</th><th>name</th>					<td></td>
				</tr>
			</thead>
			<tbody>
				...
				<?php if($fields['id']['admin_header'] == 'on') { echo '<td>'.$item['id'].'</td>';  } ?><?php if($fields['hash']['admin_header'] == 'on') { echo '<td>'.$item['hash'].'</td>';  } ?><?php if($fields['name']['admin_header'] == 'on') { echo '<td>'.$item['name'].'</td>';  } ?>					<td>	
