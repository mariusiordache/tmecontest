...
<?php include($crud_ignition_views_path . 'header.php'); ?><!-- END header -->
		...
		<div class="row-fluid">
			...
			<form method="get" class="form form-inline pull-right">
				...
				<select name="search_field" class="input-medium">
					...
					<option value="id" <?php if($search_field=='id') { echo 'selected="selected"'; } ?> >id</option><option value="item" <?php if($search_field=='item') { echo 'selected="selected"'; } ?> >item</option><option value="title" <?php if($search_field=='title') { echo 'selected="selected"'; } ?> >title</option><option value="description" <?php if($search_field=='description') { echo 'selected="selected"'; } ?> >description</option><option value="icon" <?php if($search_field=='icon') { echo 'selected="selected"'; } ?> >icon</option><option value="default_value" <?php if($search_field=='default_value') { echo 'selected="selected"'; } ?> >default_value</option>				</select>
				<label>Sort</label>				
				<select name="sort_field" class="input-small">
					<option value="">Sort by ...</option>
					<option value="id" <?php if($sort_field=='id') { echo 'selected="selected"'; } ?> >id</option><option value="item" <?php if($sort_field=='item') { echo 'selected="selected"'; } ?> >item</option><option value="title" <?php if($sort_field=='title') { echo 'selected="selected"'; } ?> >title</option><option value="description" <?php if($sort_field=='description') { echo 'selected="selected"'; } ?> >description</option><option value="icon" <?php if($sort_field=='icon') { echo 'selected="selected"'; } ?> >icon</option><option value="default_value" <?php if($sort_field=='default_value') { echo 'selected="selected"'; } ?> >default_value</option>				</select>
		...
		<table class="table table-bordered table-striped">
			<thead>
				...
				<th>id</th><th>item</th><th>title</th><th>description</th><th>icon</th><th>default_value</th>					<td></td>
				</tr>
			</thead>
			<tbody>
				...
				<?php if($fields['id']['admin_header'] == 'on') { echo '<td>'.$item['id'].'</td>';  } ?><?php if($fields['item']['admin_header'] == 'on') { echo '<td>'.$item['item'].'</td>';  } ?><?php if($fields['title']['admin_header'] == 'on') { echo '<td>'.$item['title'].'</td>';  } ?><?php if($fields['description']['admin_header'] == 'on') { echo '<td>'.$item['description'].'</td>';  } ?><?php if($fields['icon']['admin_header'] == 'on') { echo '<td>'.$item['icon'].'</td>';  } ?><?php if($fields['default_value']['admin_header'] == 'on') { echo '<td>'.$item['default_value'].'</td>';  } ?>					<td>	
