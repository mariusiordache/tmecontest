...
<?php include($crud_ignition_views_path . 'header.php'); ?><!-- END header -->
		...
		<div class="row-fluid">
			...
			<form method="get" class="form form-inline pull-right">
				...
				<select name="search_field" class="input-medium">
					...
					<option value="id" <?php if($search_field=='id') { echo 'selected="selected"'; } ?> >id</option><option value="type_id" <?php if($search_field=='type_id') { echo 'selected="selected"'; } ?> >type_id</option><option value="name" <?php if($search_field=='name') { echo 'selected="selected"'; } ?> >name</option>				</select>
				<label>Sort</label>				
				<select name="sort_field" class="input-small">
					<option value="">Sort by ...</option>
					<option value="id" <?php if($sort_field=='id') { echo 'selected="selected"'; } ?> >id</option><option value="type_id" <?php if($sort_field=='type_id') { echo 'selected="selected"'; } ?> >type_id</option><option value="name" <?php if($sort_field=='name') { echo 'selected="selected"'; } ?> >name</option>				</select>
		...
		<table class="table table-bordered table-striped">
			<thead>
				...
				<th>id</th><th>type_id</th><th>name</th>					<td></td>
				</tr>
			</thead>
			<tbody>
				...
				<?php if($fields['id']['admin_header'] == 'on') { echo '<td>'.$item['id'].'</td>';  } ?><?php if($fields['type_id']['admin_header'] == 'on') { echo '<td>'.$item['type_id'].'</td>';  } ?><?php if($fields['name']['admin_header'] == 'on') { echo '<td>'.$item['name'].'</td>';  } ?>					<td>	
