...
<?php include($crud_ignition_views_path . 'header.php'); ?><!-- END header -->
		...
		<div class="row-fluid">
			...
			<form method="get" class="form form-inline pull-right">
				...
				<select name="search_field" class="input-medium">
					...
					<option value="id" <?php if($search_field=='id') { echo 'selected="selected"'; } ?> >id</option><option value="user_id" <?php if($search_field=='user_id') { echo 'selected="selected"'; } ?> >user_id</option><option value="settings_id" <?php if($search_field=='settings_id') { echo 'selected="selected"'; } ?> >settings_id</option><option value="value" <?php if($search_field=='value') { echo 'selected="selected"'; } ?> >value</option>				</select>
				<label>Sort</label>				
				<select name="sort_field" class="input-small">
					<option value="">Sort by ...</option>
					<option value="id" <?php if($sort_field=='id') { echo 'selected="selected"'; } ?> >id</option><option value="user_id" <?php if($sort_field=='user_id') { echo 'selected="selected"'; } ?> >user_id</option><option value="settings_id" <?php if($sort_field=='settings_id') { echo 'selected="selected"'; } ?> >settings_id</option><option value="value" <?php if($sort_field=='value') { echo 'selected="selected"'; } ?> >value</option>				</select>
		...
		<table class="table table-bordered table-striped">
			<thead>
				...
				<th>id</th><th>user_id</th><th>settings_id</th><th>value</th>					<td></td>
				</tr>
			</thead>
			<tbody>
				...
				<?php if($fields['id']['admin_header'] == 'on') { echo '<td>'.$item['id'].'</td>';  } ?><?php if($fields['user_id']['admin_header'] == 'on') { echo '<td>'.$item['user_id'].'</td>';  } ?><?php if($fields['settings_id']['admin_header'] == 'on') { echo '<td>'.$item['settings_id'].'</td>';  } ?><?php if($fields['value']['admin_header'] == 'on') { echo '<td>'.$item['value'].'</td>';  } ?>					<td>	
