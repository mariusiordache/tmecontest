...
<?php include($crud_ignition_views_path . 'header.php'); ?><!-- END header -->
		...
		<div class="row-fluid">
			...
			<form method="get" class="form form-inline pull-right">
				...
				<select name="search_field" class="input-medium">
					...
					<option value="id" <?php if($search_field=='id') { echo 'selected="selected"'; } ?> >id</option><option value="date_created" <?php if($search_field=='date_created') { echo 'selected="selected"'; } ?> >date_created</option><option value="email" <?php if($search_field=='email') { echo 'selected="selected"'; } ?> >email</option><option value="username" <?php if($search_field=='username') { echo 'selected="selected"'; } ?> >username</option><option value="password" <?php if($search_field=='password') { echo 'selected="selected"'; } ?> >password</option><option value="name" <?php if($search_field=='name') { echo 'selected="selected"'; } ?> >name</option>				</select>
				<label>Sort</label>				
				<select name="sort_field" class="input-small">
					<option value="">Sort by ...</option>
					<option value="id" <?php if($sort_field=='id') { echo 'selected="selected"'; } ?> >id</option><option value="date_created" <?php if($sort_field=='date_created') { echo 'selected="selected"'; } ?> >date_created</option><option value="email" <?php if($sort_field=='email') { echo 'selected="selected"'; } ?> >email</option><option value="username" <?php if($sort_field=='username') { echo 'selected="selected"'; } ?> >username</option><option value="password" <?php if($sort_field=='password') { echo 'selected="selected"'; } ?> >password</option><option value="name" <?php if($sort_field=='name') { echo 'selected="selected"'; } ?> >name</option>				</select>
		...
		<table class="table table-bordered table-striped">
			<thead>
				...
				<th>id</th><th>date_created</th><th>email</th><th>username</th><th>password</th><th>name</th>					<td></td>
				</tr>
			</thead>
			<tbody>
				...
				<?php if($fields['id']['admin_header'] == 'on') { echo '<td>'.$item['id'].'</td>';  } ?><?php if($fields['date_created']['admin_header'] == 'on') { echo '<td>'.$item['date_created'].'</td>';  } ?><?php if($fields['email']['admin_header'] == 'on') { echo '<td>'.$item['email'].'</td>';  } ?><?php if($fields['username']['admin_header'] == 'on') { echo '<td>'.$item['username'].'</td>';  } ?><?php if($fields['password']['admin_header'] == 'on') { echo '<td>'.$item['password'].'</td>';  } ?><?php if($fields['name']['admin_header'] == 'on') { echo '<td>'.$item['name'].'</td>';  } ?>					<td>	
