...
<?php include($crud_ignition_views_path . 'header.php'); ?><!-- END header -->
		...
		<div class="row-fluid">
			...
			<form method="get" class="form form-inline pull-right">
				...
				<select name="search_field" class="input-medium">
					...
					<option value="id" <?php if($search_field=='id') { echo 'selected="selected"'; } ?> >id</option><option value="item_id" <?php if($search_field=='item_id') { echo 'selected="selected"'; } ?> >item_id</option><option value="path" <?php if($search_field=='path') { echo 'selected="selected"'; } ?> >path</option><option value="is_selected" <?php if($search_field=='is_selected') { echo 'selected="selected"'; } ?> >is_selected</option>				</select>
				<label>Sort</label>				
				<select name="sort_field" class="input-small">
					<option value="">Sort by ...</option>
					<option value="id" <?php if($sort_field=='id') { echo 'selected="selected"'; } ?> >id</option><option value="item_id" <?php if($sort_field=='item_id') { echo 'selected="selected"'; } ?> >item_id</option><option value="path" <?php if($sort_field=='path') { echo 'selected="selected"'; } ?> >path</option><option value="is_selected" <?php if($sort_field=='is_selected') { echo 'selected="selected"'; } ?> >is_selected</option>				</select>
		...
		<table class="table table-bordered table-striped">
			<thead>
				...
				<th>id</th><th>item_id</th><th>path</th><th>is_selected</th>					<td></td>
				</tr>
			</thead>
			<tbody>
				...
				<?php if($fields['id']['admin_header'] == 'on') { echo '<td>'.$item['id'].'</td>';  } ?><?php if($fields['item_id']['admin_header'] == 'on') { echo '<td>'.$item['item_id'].'</td>';  } ?><?php if($fields['path']['admin_header'] == 'on') { echo '<td>'.$item['path'].'</td>';  } ?><?php if($fields['is_selected']['admin_header'] == 'on') { echo '<td>'.$item['is_selected'].'</td>';  } ?>					<td>	
